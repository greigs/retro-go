/**
 * Experimental layer class to do play with pixel in an off-screen buffer before
 *painting to the display
 *
 * Requires GFX_Lite
 *
 * Codetastic 2020
        ...
        for (int y = 0; y < _height; y++) {
                for (int x = 0; x < _width; x++)
                {
                        ...
                        if (_pixel != transparency_colour) {
                                matrix->drawPixelRGB888( x, y, _pixel.r,
_pixel.g, _pixel.b);
                        }
...
void GFX_Layer::drawCentreText(const char* buf, textPosition textPos,
                               const GFXfont* f, CRGB color, int yadjust) {
  int16_t x1, y1;
  uint16_t w, h;
  ...
  // Serial.printf("The width of the text is %d pixels, the height is %d
  // pixels.\n", w,h);
  ...
  /*
   ...
   identified the top-left corner of the character cell, with new fonts the
   cursor position indicates the baseline — the bottom-most row — of subsequent
   text. Characters may vary in size and width, and don’t necessarily begin at
   the exact cursor column (as in below, this character starts one pixel left of
   the cursor, but others may be on or to the right of it).
   */
  ...
  /* As we can't reliable know what is the actual FIRST and last 'lit' pixel, we
     need to check what was printed to the layer.*/
  {
    int wstart = 0;
  ...
  //	setCursor(0,16);
  setTextColor(this->color565(
      color.r, color.g,
      color.b));  // Need to confirm from FastLed CRGB to adafruit 565
  print(buf);
 ...
 * Useful if you want to make sure text / graphics IS in the centre of the
 * display.
 */
...
void GFX_Layer::reduceBrightness(uint8_t value) {
  for (int y = 0; y < _height; y++) {
    for (int x = 0; x < _width; x++) {
      pixels->data[y][x].nscale8(value);
    }
...
/*
 * Display the foreground pixels if they're not the background/transparent
 * color. If not, then fill with whatever is in the background.
 *
 * writeToBg = write the result back to the _bgLayer, and not directly to the
 * output device!
 * 			   -> no need to do a subsequent bgLayer.display()
 * otherwise.
 */
void GFX_LayerCompositor::Stack(GFX_Layer& _bgLayer, GFX_Layer& _fgLayer,
                                bool writeBackToBg) {
  for (int y = 0; y < _fgLayer.getHeight(); y++) {
    for (int x = 0; x < _fgLayer.getWidth(); x++) {
      ...
      if (_fgLayer.pixels->data[y][x] ==
          _fgLayer.transparency_colour)  // foreground is transparent, show the
                                         // _bgLayer colors
      {
        if (writeBackToBg)  // write the foreground to the background layer...
                            // perhaps so we can do stuff later with the
                            // _fgLayer.
          _bgLayer.pixels->data[y][x] = _bgLayer.pixels->data[y][x];
        else

          callback(x, y, _bgLayer.pixels->data[y][x].r,
                   _bgLayer.pixels->data[y][x].g,
        ...
        if (writeBackToBg)  // write the foreground to the background layer...
          ...
          callback(x, y, _fgLayer.pixels->data[y][x].r,
                   _fgLayer.pixels->data[y][x].g,
...
/*
 * Where the foreground pixels are not the background/transparent color,
 * populate with whatever is in the background.
 */
  ...
  for (int y = 0; y < _fgLayer.getHeight(); y++) {
    for (int x = 0; x < _fgLayer.getWidth(); x++) {
      ...
      if (_fgLayer.pixels->data[y][x] != _fgLayer.transparency_colour) {
        callback(x, y, _bgLayer.pixels->data[y][x].r,
                 _bgLayer.pixels->data[y][x].g, _bgLayer.pixels->data[y][x].b);
...
void GFX_LayerCompositor::StackWithThreshold(GFX_Layer& _bgLayer, GFX_Layer& _fgLayer, uint8_t threshold, bool writeBackToBg) {
  for (int y = 0; y < _fgLayer.getHeight(); y++) {
    for (int x = 0; x < _fgLayer.getWidth(); x++) {
      CRGB fgPixel = _fgLayer.pixels->data[y][x];
      uint8_t brightness = (fgPixel.r + fgPixel.g + fgPixel.b) / 3;

      if (brightness < threshold) {  // foreground is considered transparent
        if (writeBackToBg) {
          _bgLayer.pixels->data[y][x] = _bgLayer.pixels->data[y][x];
        } else {
          callback(x, y, _bgLayer.pixels->data[y][x].r,
      ...
      } else {  // foreground is not transparent
        if (writeBackToBg) {
          _bgLayer.pixels->data[y][x] = fgPixel;
        } else {
          callback(x, y, fgPixel.r, fgPixel.g, fgPixel.b);
        }
      }
    }
  }
...
void GFX_LayerCompositor::Blend(GFX_Layer& _bgLayer, GFX_Layer& _fgLayer,
                                uint8_t ratio) {
  CRGB _pixel = 0;
  ...
  for (int y = 0; y < _fgLayer.getHeight(); y++) {
    for (int x = 0; x < _fgLayer.getWidth(); x++) {
      ...
      //_pixel = blend(_bgLayer.pixels->data[y][x], _fgLayer.pixels->data[y][x],
      //ratio);
