...
class Motion {
 ...
 protected:
  ...
  void boundaryCheck(float boundaryForce = BOUNDARY_FORCE) {
    outOfBoundary = false;
    float marginX = BORDER_BUFFER;
    float marginTop = BORDER_BUFFER;
    float marginBottom = BORDER_BUFFER;
#if defined(PANEL_CYD_TFT)
#ifndef CYD_KEEP_INSIDE_MARGIN_X
#define CYD_KEEP_INSIDE_MARGIN_X 0
#endif
#ifndef CYD_KEEP_INSIDE_MARGIN_TOP
#define CYD_KEEP_INSIDE_MARGIN_TOP 0
#endif
#ifndef CYD_KEEP_INSIDE_MARGIN_BOTTOM
#define CYD_KEEP_INSIDE_MARGIN_BOTTOM 0
#endif
    if (boundaryForce > BOUNDARY_FORCE) {
      marginX = CYD_KEEP_INSIDE_MARGIN_X * PHYSICS_SCALE;
      marginTop = CYD_KEEP_INSIDE_MARGIN_TOP * PHYSICS_SCALE;
      marginBottom = CYD_KEEP_INSIDE_MARGIN_BOTTOM * PHYSICS_SCALE;
    }
#endif
    ...
    if (pos.x > xResolution - marginX) {
      applyForce(PVector(-boundaryForce, 0));
      outOfBoundary = true;
    }
    if (pos.y > yResolution - marginBottom) {
      applyForce(PVector(0, -boundaryForce));
      outOfBoundary = true;
    }
  }
